...
Conversations.prototype.get = function (options, callback) {
  if (typeof options === 'function') {
    callback = options;
    options = {};
  }
  options = defaults(options, { id: 1 });
  debug('requesting conversation [id %d] ..', options.id);
  request
    .get('https://api.helpscout.net/v1/conversations/' + options.id + '.json')
    .auth(this.apiKey, 'X')
    .query(options)
